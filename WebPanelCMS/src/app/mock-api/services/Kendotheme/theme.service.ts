 ...
 *
 * @export
 * @class KendoThemeService
 * @implements {OnDestroy}
 */
@Injectable({
  providedIn: 'root'
})
export class KendoThemeService implements OnDestroy {
  ...
  */
      ...
      rendererFactory: RendererFactory2)
  {
    ...
    this.settings = this.getLocalStorageItem(KendoThemeService.SETTINGS_KEY);
    if (!this.settings)
    {
  ...
  {
    const result: boolean = this.document.defaultView?.matchMedia(KendoThemeService.SYSTEM_DARK_MODE_MEDIA_QUERY).matches!;
    return result;
  }
  ...
  private setMode(darkMode: boolean): void
  {
    if (darkMode)
    {
      this.renderer.removeClass(this.document.body, KendoThemeService.LIGHT_MODE_CSS_CLASS_NAME);
      this.setKendoUiControlsMode('kendoui-dark.css');
    }
    else
    {
      this.renderer.addClass(this.document.body, KendoThemeService.LIGHT_MODE_CSS_CLASS_NAME);
  ...
  private startSystemModeSynchronization(): void
  {
    if ((!this.destructionSubject) && (this.document.defaultView))
    {
      ...
      fromEvent(this.document.defaultView.matchMedia(KendoThemeService.SYSTEM_DARK_MODE_MEDIA_QUERY), 'change').pipe(
  ...
  setSystemMode(): void
  {
    ...
    this.setMode(darkMode);
    this.startSystemModeSynchronization();
    this.removeLocalStorageItem(KendoThemeService.SETTINGS_KEY);
  }
  ...
  setUserDefinedMode(darkMode: boolean): void
  {
    ...
    this.setLocalStorageItem(KendoThemeService.SETTINGS_KEY, this.settings);
  }
