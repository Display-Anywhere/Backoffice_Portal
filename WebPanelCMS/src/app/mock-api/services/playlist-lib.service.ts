...
export class PlaylistLibService {
  ...
  SaveMasterScheduleName(id, name, dfclientId,chkOverwritemanual,mediaType) {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    const params = JSON.stringify({
      id, name,dfclientId,chkOverwritemanual,mediaType
    });
    return this.http.post(this.cApi.SaveMasterScheduleName, params, { headers })
      .pipe((data => data));
  }
  MasterSchedulePublication(id) {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    const params = JSON.stringify({
      id
    });
    return this.http.post(this.cApi.MasterSchedulePublication, params, { headers })
      .pipe((data => data));
  }
}
