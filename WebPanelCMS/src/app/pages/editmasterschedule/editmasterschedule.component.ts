...
export class EditmasterscheduleComponent implements OnInit {
  ...
  dt = new Date(2000, 11, 31, 0,0);
  dt2 = new Date(2000, 11, 31, 23,59);
  cmbSearchCustomer = "0";
  cmbSearchFormat = 0;
  cmbSearchPlaylist = 0;
  ...
  constructor(
    ...
    configTime: NgbTimepickerConfig, library: FaIconLibrary
  ) {
  ...
  initSFform(){
    this.SFform = this.formBuilder.group({
      CustomerId: [localStorage.getItem('selecteddfClientId'), Validators.required],
...
if (errorFound==="Yes"){
  ...
  FillClient() {
    ...
    q =
      ...
      i +
      ', ' +
      localStorage.getItem('selecteddfClientId') +
      ',' +
      ...
      .subscribe(
        (data) => {
          ...
          this.loading = false;
            ...
            this.cmbCustomer_MasterSchedule=localStorage.getItem('selecteddfClientId')
            this.cid=localStorage.getItem('selecteddfClientId')
  ...
  FillFormat() {
    var qry = "";
    var innerQry=""
    let mType= localStorage.getItem('mastermediatype')
    if ((mType == 'Signage') || (mType == 'Video') || (mType == 'Templates')){
      innerQry= " and sf.mediatype in('Signage','Video','Templates') "
    }
    else if ((mType == 'Audio')){
      innerQry= " and sf.mediatype in('Audio Copyright','Audio DirectLicence', 'Audio') "
    }
    else{
      innerQry= " and sf.mediatype in('"+mType+"') "
    }
  ...
  async onChangeCustomer(deviceValue) {
    ...
    this.SFform.get('startTime').setValue(this.dt);
    this.time = {
      hour: this.dt2.getHours(),
      minute: this.dt2.getMinutes(),
      second: 0,
    };
    this.SFform.get('EndTime').setValue(this.dt2);
    ...
    this.sfService
      ...
      .subscribe(
        (data) => {
          ...
          if ((mtype == 'Signage')  || (mtype == 'Templates')){
            mtype='Signage'
          }
          if (ptype === '') {
...
OpenViewContent(modalName, url,oType,MediaType){
    ...
    GetMasterScheduleDetail(id){
      ...
      this.sfService.GetMasterScheduleDetail(id,this.cmbCustomer_MasterSchedule,localStorage.getItem('UserId')).pipe().subscribe(async (data) => {
            ...
            if (objData.response=="0"){
              ...
              this.CustomSchedulePlaylist=[]
              localStorage.setItem("MasterScheduleTokenInfoList","0")
              return
            }
            ...
            if (player.length==0){
              ...
              localStorage.setItem("MasterScheduleTokenInfoList",JSON.stringify(this.MasterScheduleTokenInfoList))
            }
            this.loading = false;
  ...
  console.log(this.SFform.value)
    ...
    gotbacktoMasters(){
      this.router.navigate(['/general/medialibrary/']);
    }
}
