...
@Component({
  selector: 'app-masterschedule-normalad',
  templateUrl: './masterschedule-normalad.component.html',
  styleUrls: ['./masterschedule-normalad.component.scss']
})
export class MasterscheduleNormaladComponent {
  ...
  onSubmitAd = function () {
    ...
    let objTokenInfo= localStorage.getItem("MasterScheduleTokenInfoList")
    if (objTokenInfo=="0"){
      this.toastr.error("Please add devices in master schedule");
      return;
    }
    this.TokenSelected=[]
    let TokenInfo= JSON.parse(objTokenInfo)
  ...
  FillClientList() {
    ...
    if (this.auth.IsAdminLogin$.value == true) {
      ...
      str = str + " where DFClients.CountryCode in(" + this.CountrySelected + ")  and (dbtype='"+localStorage.getItem('DBType')+"' or dbtype='Both') and DFClients.DealerDFClientID= " + localStorage.getItem('selecteddfClientId') + "    ";
      str = str + " union all select distinct DFClients.DFClientID,DFClients.ClientName from DFClients ";
      str = str + " inner join AMPlayerTokens on DFClients.DfClientid=AMPlayerTokens.Clientid ";
      str = str + " where DFClients.CountryCode in(" + this.CountrySelected + ")  and (dbtype='"+localStorage.getItem('DBType')+"' or dbtype='Both') and DFClients.MainDealerid= " + localStorage.getItem('selecteddfClientId') + "    ";
  ...
  FillSearchClientList() {
    ...
    this.aService.FillCombo(str).pipe()
      .subscribe(data => {
        ...
        this.Adform.get('CustomerLst').setValue(localStorage.getItem('selecteddfClientId'));
        this.SearchAd()
        this.FillCategory();
...
*/
  ...
  FillCountry() {
    ...
    if (this.IsEditClick == "No") {
      ...
      strCou = strCou + " where DFClients.CountryCode is not null and DFClients.DealerDFClientID= " + localStorage.getItem('selecteddfClientId') + "    ";
      strCou = strCou + " union all select distinct DFClients.DFClientID  from DFClients ";
      strCou = strCou + " inner join AMPlayerTokens on DFClients.DfClientid=AMPlayerTokens.Clientid ";
      strCou = strCou + " where DFClients.CountryCode is not null and DFClients.MainDealerid= " + localStorage.getItem('selecteddfClientId') + "    ";
      strCou = strCou + " ) ";
  ...
  OpenViewContent(modalName, url,MediaType){
    ...
    async openAddNewAdsModal(modalName){
      let TokenInfo= localStorage.getItem("MasterScheduleTokenInfoList")
      if (TokenInfo=="0"){
        this.toastr.error("Please add devices in master schedule");
        return;
      }
