...
export class DownloadTemplateComponent  implements OnInit {
  ...
  cmbGenre = "0";
  cmbFolder = "0";
  CustomerId = localStorage.getItem('selecteddfClientId')
...
if (this.cmbGenre=='0'){
  return;
    ...
    this.loading = true;
    this.serviceLicense.GetTemplates(this.CustomerId, this.cmbGenre, this.SearchCDate, '').pipe()
      .subscribe(data => {
        var returnData = JSON.stringify(data);
...
let payloadTemplateSelected=[]
this.TemplateSelected.forEach(item=>{
  let obj = this.TemplateList.filter(o=>o.id==item)
  let TemplateItem = {}
  TemplateItem["TemplateName"] = obj[0].name;
  TemplateItem["Url"] = obj[0].url;
  TemplateItem["id"] = obj[0].id;
  TemplateItem["duration"] = obj[0].duration;
  TemplateItem["Refersh"] = parseInt(obj[0].duration)*2;
  TemplateItem["IsAnnouncement"] = false;
  payloadTemplateSelected.push(TemplateItem)
});

    this.loading = true;

    this.serviceLicense.DownloadTemplates_new(this.CustomerId,this.cmbGenre,this.cmbFolder,payloadTemplateSelected).pipe()
      .subscribe(data => {
        var returnData = JSON.stringify(data);
        var obj = JSON.parse(returnData);
        if (obj.Responce=="1"){
