...
import { ExcelServiceService } from 'app/mock-api/services/excel-service.service';
import { FormatSettings } from '@progress/kendo-angular-dateinputs';

@Component({
  selector: 'app-client-player-list',
  templateUrl: './client-player-list.component.html',
  styleUrls: ['./client-player-list.component.css'],
...
open(content, tid) {
  ...
  this.modalService.open(content, {centered: true, size: 'lg', windowClass: 'tokenmodal' });
}
async onChangeCustomer(deviceValue) {
  ...
  await this.FillDeviceLastStatus(this.cmbCustomerId)
}
...
FillData(data) {
  ...
  this.gridViewList=[]
  this.gridViewList = this.TokenList 
  this.RecordsFilterList=[]
  this.RecordsFilterList=[
    {
      value:"All",
      text:"All"
  ...
  if (this.isSanitizerActive==false){
    this.RecordsFilterList= this.RecordsFilterList.filter(o => o.value !='Sanitizer')
  }
  if (this.h_info_Active==false){
    this.RecordsFilterList= this.RecordsFilterList.filter(o => o.value !='HotelTv')
  }
  if (this.E_Link_Active==false){
    this.RecordsFilterList= this.RecordsFilterList.filter(o => o.value !='WhiteBoard')
  }
  if (this.KPN_Active==false){
    this.RecordsFilterList= this.RecordsFilterList.filter(o => o.value !='KPN')
  }
}
...
OpenFullImageModal(ObjModal, url) {
  ...
  this.modalService.open(ObjModal, { size: 'lg',centered: true });
}
      ...
      (data) => {
       ...
       /*  const obj:SortEvent   ={
        ...
        if (this.ActiveTokenList.length != 0) {
          this.modalService.open(modalContant, { size: 'lg',centered: true });
        } else {
...
OpenGroupsModal(gModal) {
  ...
  this.modalService.open(gModal, { size: 'lg',centered: true });
}
...
OpenOpeningHoursModal(gModal) {
  ...
  localStorage.setItem('tcid', this.cid);
  this.FilterValue_For_Reload = 'Regsiter';
  this.modalService.open(gModal, { size: 'lgx' ,centered: true});
}
    ...
    .subscribe(
      (data) => {
        ...
        if (this.CustomerMediaTypeList.length >1){
          this.modalService.open(modalMediaTypeDefault,{centered: true})
        }
...
openphinfo(philipsinfoModal){
  this.modalService.open(philipsinfoModal, { size: 'lg',centered: true });
...
DisplayGroupRooms(mdl){
  ...
  this.modalService.open(mdl,{centered: true})
}
onChangerType(){
  this.SelectedMeetingArray =[]
  this.RoomSignagePlaylist=[]
