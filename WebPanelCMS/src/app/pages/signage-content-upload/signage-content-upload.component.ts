...
@Component({
  selector: 'app-signage-content-upload',
  templateUrl: './signage-content-upload.component.html',
  styleUrls: ['./signage-content-upload.component.css']
})
export class SignageContentUploadComponent implements OnInit {
...
var Item_TitleId="";
   ...
   this.CustomerId = localStorage.getItem('selecteddfClientId');
   await this.onChangeCustomer(this.CustomerId)
  ...
  */
  ...
  onChangeFolder(id){
    ...
    this.resIsAutoDelete=false;
    var sd1= new Date()
      this.dtpDeleteDate=new Date()
    var ArrayItem = {};
    var fName = "";
    ArrayItem["Id"] = this.cmbFolder;
    ArrayItem["DisplayName"] = "";
    //this.cmbFolder = id;
    console.log(ArrayItem)
    this.GetJSONFolderRecord(ArrayItem);
    console.log(this.NewfList)
...
GetFolderContent(mdl){
  ...
  this.serviceLicense.GetFolderContent(this.cmbFolder,this.CustomerId).pipe()
      .subscribe(data => {
        ...
        this.loading = false;
        if (mdl !="null"){
          this.modalService.open(mdl,{centered: true, size: 'lg' });
        }
      },
        error => {
          this.toastr.error("Apologies for the inconvenience.The error is recorded.", '');
  ...
  if (genreId =="324"){
    ...
    if (oType=="LS"){
      this.modalService.open(modalName, {
        size: 'Template',centered: true
      }); 
    }
    if (oType=="PT"){
      this.modalService.open(modalName,{
        size: 'PT-Template',centered: true
      }); 
    }
