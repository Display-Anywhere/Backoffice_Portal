...
@Component({
  selector: 'app-rep-machine-dispenser-change-log',
  templateUrl: './rep-machine-dispenser-change-log.component.html',
  styleUrls: ['./rep-machine-dispenser-change-log.component.css']
})
export class RepMachineDispenserChangeLogComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  SearchPlayedTitleSummary() {
    ...
    this.rerender();
if (this.tokenid=="0"){
  this.file_Name = this.Client_Name + "_"  + FromDate.toDateString() + "_" + ToDate.toDateString() + "_DispenserLogs";
}
else{
  this.file_Name = this.Client_Name + "_" + this.tokenid + "_" + FromDate.toDateString() + "_" + ToDate.toDateString() + "_DispenserLogs";
}
this.DataTableSettings();

    this.loading = true;
 
    this.rService.FillMachineLogs(FromDate.toDateString(), ToDate.toDateString(), this.tokenid,this.cid,true).pipe()
      .subscribe(data => {
        var returnData = JSON.stringify(data);
