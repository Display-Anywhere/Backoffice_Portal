...
this.DataTableSettings();
      ...
      .subscribe(data => {
        ...
        if (obj.Responce == '1') {
          ...
          if (this.plLastName == '') {
            ...
            this.NewName = this.playlistform.value.plName + '  (' + this.plLastName;
          }
          this.plLastName = '';
  ...
  FillFormat() {
    ...
    qry = qry + " (dbtype='" + localStorage.getItem('DBType') + "' or dbtype='Both') and  (st.dfclientid=" + this.cmbCustomer + " OR sf.dfclientid=" + this.cmbCustomer + ") and sf.mediatype='"+this.cmbCustomerMediaType+"' group by  sf.formatname";
  ...
  AddSong(UpdateModel) {
    ...
    this.loading = true;
    this.pService.AddPlaylistSong(this.PlaylistSelected, this.SongsSelected, "SFPlaylist", NewList[0].IsDuplicate).pipe()
      .subscribe(data => {
        var returnData = JSON.stringify(data);
        var obj = JSON.parse(returnData);
        this.loading = false;
...
if (this.cmbCustomerMediaType === ''){
  ...
  onSubmitNewFormat() {
    ...
    this.pService.SaveFormat(this.formatid, this.NewFormatName, this.cmbCustomer, this.cmbCustomerMediaType).pipe()
      .subscribe(data => {
        var returnData = JSON.stringify(data);
        var obj = JSON.parse(returnData);
        if (obj.Responce != "-2") {
  ...
  onPlaylistSettingClick(id, mContent, chkMu, chkFixed, Mixed, tids, spName, Duplicate) {
    this.pid = id;
    this.txtDelPer = "0";
    this.chkMute = chkMu;
  ...
  SettingPlaylist(UpdateModel) {
    this.loading = true;
    this.pService.SettingPlaylist(this.pid, this.chkMute, this.chkFixed, this.chkMixed, this.chkDuplicate).pipe()
      .subscribe(data => {
        var returnData = JSON.stringify(data);
        var obj = JSON.parse(returnData);
        if (obj.Responce == "1") {
...
//this.rerender();
  ...
  SaveImageTimeInterval_API(type) {
    this.loading = true;
    this.pService.SaveImageTimeInterval(this.ImageTimeInterval).pipe()
      .subscribe(data => {
        const returnData = JSON.stringify(data);
